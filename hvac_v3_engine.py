...
HVAC_PRESETS = {
    ...
    "Custom": {"COP_COOL_NOM": 4.0, "COP_HEAT_NOM": 3.2, "FAN_EFF": 0.65},
}
...
def _load_base_weather(
    ...
    if weather_df is not None:
        ...
        return base_weather, weather_summary_dict(base_weather, "uploaded_dataframe", None)
    if weather_mode == "epw" and epw_path:
        base_weather = read_epw_daily(epw_path)
        return base_weather, weather_summary_dict(base_weather, "epw", epw_path)
    if weather_mode == "csv" and csv_path:
        base_weather = read_weather_csv_daily(csv_path)
        return base_weather, weather_summary_dict(base_weather, "csv", csv_path)
    if weather_mode == "uploaded" and epw_path:
        base_weather = read_weather_auto_daily(epw_path)
        return base_weather, weather_summary_dict(base_weather, "uploaded", epw_path)
    base_weather = synthetic_daily_weather(random_state)
    return base_weather, weather_summary_dict(base_weather, "synthetic", None)
...
def simulate_baseline_no_degradation(
    ...
    for step in range(cfg.years * steps_per_year):
        ...
        E_period = (P_hvac + P_fan) * duration_hours
...
def cooling_heating_loads(bldg: BuildingSpec, cfg: HVACConfig, derived: Dict[str, float], T_mean: float, RH_mean: float, GHI_mean: float, T_sp: float, occ: float, doy: int) -> Dict[str, float]:
    ...
    q_cool_solar = (cfg.SOLAR_COOL_FACTOR * q_cool_des * ghi_norm * max(math.sin(math.pi * doy / 365.0), 0.0)) if solar_enabled else 0.0
...
def evaluate_controls(bldg: BuildingSpec, cfg: HVACConfig, derived: Dict[str, float], T_mean: float, RH_mean: float, GHI_mean: float, occ: float, year_frac: float, doy: int, rf: float, dust: float, T_sp: float, af: float, duration_hours: float = 24.0) -> Dict[str, float]:
    ...
    return {"rf_next": rf_next, "dust_next": dust_next, "dp_next": dp_next, "deg_next": deg_next, "cop": current_cop, "Q_cool_kw": loads["Q_cool_kw"], "Q_heat_kw": loads["Q_heat_kw"], "Q_HVAC_kw": loads["Q_HVAC_kw"], "mode": mode, "P_tot": P_tot, "P_fan": P_fan, "P_hvac": P_hvac, "E_day": E_period, "E_period": E_period, "co2": co2, "comfort_dev": comfort_dev, "objective": J, "people": loads.get("people", 0.0), "internal_kw": loads.get("internal_kw", 0.0)}
...
def simulate_combo(
    ...
    for step in range(cfg.years * steps_per_year):
        ...
        T_mean, T_max, RH_mean, GHI_mean, occ = climate_and_operation_for_day(day_index, base_weather, climate_name, schedule_profile)
...
def run_scenario_model(
    ...
    base_weather, weather_meta = _load_base_weather(weather_mode, epw_path, csv_path, weather_df, random_state)
